...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    /**
     * 自定义了一个 Spring Security 过滤器 TokenAuthenticationFilter，它继承了 OncePerRequestFilter，确保每个请求只被过滤一次。
     * 在重写的 doFilterInternal() 方法中来定义过滤器处理逻辑，首先，从请求头中获取 key 为 Authorization 的值，判断是否以 Bearer 开头，若是，截取出 Token, 对其进行解析，并对可能抛出的异常做出不同的返参。
