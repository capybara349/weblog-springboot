...
import com.capybara349.weblog.admin.convert.CommentConvert;
...
import com.capybara349.weblog.admin.service.AdminCommentService;
...
public class AdminCommentServiceImpl implements AdminCommentService {
    ...
    /**
     * 删除评论
     * 首先，在方法上添加 @Transactional(rollbackFor = Exception.class) 事务回滚注解，因为删除评论逻辑中，可能会多次更新数据库，需要保证整个操作的原子性；
     * 接着，查询想要删除的评论，通过 replayCommentId 字段值是否为空，来判断其类型，是一级评论呢，还是二级评论；
     * 如果是一级评论，除了将该记录本身删除掉外，还需要将其所有子评论删除；
     * 如果是二级评论，删除本身的同时，需要判断该评论是否被会回复了，如果有被回复，需要将回复的评论删除掉，注意，回复的评论被回复了，均需要删除，这里使用到了递归调用，直到下级数据全部删除成功为止，避免留下脏数据。
