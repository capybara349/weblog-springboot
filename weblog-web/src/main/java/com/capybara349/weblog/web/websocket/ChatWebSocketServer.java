package com.capybara349.weblog.web.websocket;
...
import com.capybara349.weblog.web.model.vo.chatroom.*;
import com.capybara349.weblog.web.enums.ChatRoomMessageTypeEnum;
import com.capybara349.weblog.web.utils.SpringContext;
...
public class ChatWebSocketServer {
    ...
    public void onOpen(Session session) {
        ...
        // 返回自己的 sessionKey 给前端（只发送当前会话）
        sendMessage(session, buildSessionIdMessage(sessionKey));

        // 广播系统消息，告诉所有人，有用户加入了聊天室
        broadcastMessage(buildMessage(ChatRoomMessageTypeEnum.SYSTEM.getCode(), nickname, null, "加入了聊天室", sessionKey));
        // 广播在线用户列表
    ...
    public void onClose(Session session) {
        ...
        if (SESSION_MAP.containsKey(sessionKey)) {
            ...
            broadcastMessage(buildMessage(ChatRoomMessageTypeEnum.SYSTEM.getCode(), nickname, null, "离开了聊天室", sessionKey));
            // 广播在线用户列表
            broadcastOnlineUsers();
        }
    }
    ...
    public void onMessage(String message, Session session) {
        ...
        // 广播聊天消息，通知所有在线用户（携带发送者的 sessionKey）
